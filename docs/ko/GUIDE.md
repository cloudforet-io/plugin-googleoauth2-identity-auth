...
구글 워크스페이스를 사용하는 회사의 경우 클라우드포레의 OAuth 플러그인을 활용하면 사용자의 로그인 프로세스를 간소화하는데 도움이 됩니다.  
...
이제 OAuth 클라이언트 ID 생성이 완료 되었으니 Domain 검증을 진행합니다.
...
이후, [1. OAuth 클라이언트 ID 생성](./GUIDE.md/#1-oauth-클라이언트-id-생성)에서 획득한 [**클라이언트 ID**]를   
